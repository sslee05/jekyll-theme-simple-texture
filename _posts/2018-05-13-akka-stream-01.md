...
Source, Sink, Flow들은 위에 했던 내용을 보면 그림으로 나타내지 안더라도 위의 그림이 생각 났으리라, 그러나 BidiFlow는 글로 설명하기 보다는 그림으로 나타내면 확실히 이해하기가 편하다.  
BidiFlow는 위의 그림처럼 2개의 출력 포트와 2개의 입력 포트가 있다. 이를 이용하면 다양한 모양의 Stream설계도를 그릴 수 있다.  
위의 예제(json String -> Event)를 json byte string -> Event -> json byte String으로 변환 할때 
