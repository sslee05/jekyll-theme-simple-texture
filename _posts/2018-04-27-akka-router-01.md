...
2. Routee 만들기  
별다른 것 이 없다.  
Message를 받으면 다른 Actor에게 응답을 보낸다.
{% highlight scala %}
...
class MyRouteeActor(actorRef: ActorRef) extends Actor with ActorLogging {
  
  def receive = {
    case MyMessage(value) =>
      actorRef ! MyReplyMessage(value)
  }
}

object MyRouteeActor {
  def props(actorRef: ActorRef) = Props(new MyRouteeActor(actorRef))
  def name = "myRouteeActor" 
}
{% endhighlight %}

3. TestKit 만들기  
ActorSystem은 비데몬성으로 실행 되므로 test를 끝난 경우에도 process가 중지 되지 않는다.  
따라서 test가 끝나면 hock메소드에 system를 중지하게 한 class이다.  
아래 router생성부분을 보면 FromConfig.props에 해당 Routee 생성의 Props를 감싼다.  
이게 끝이다.   
설정 file정보를 읽지 않고 code로 구성시 아래 예제에서는 BalancingPool(routee갯수).prop로 Routee 생성의 Props를 감싸면 된다.  
참고로 보낼 message에 Broadcast를 감싸서 보내면 모든 routee에 보내지게 된다.  
여기서 모든 routee들은 위의 설정에 의하여 2개가 되겠다.
...
class LocalPoolTest extends TestKit(ActorSystem("localPool",ConfigFactory.load("local-pool"))) 
  ...
  "router by pool in local" must {
    ...
    "using router by config" in {
      ...
      //router 생성 
      val router = system.actorOf(FromConfig.props(MyRouteeActor.props(testActor)),"localRouter")
    ...
    "using router by code" in {
      ...
      //router 생성 
      val router = system.actorOf(RoundRobinPool(5).props(MyRouteeActor.props(testActor)),"localRouter")
...
아래 같은 설정을 해야 한다. 그러면 sbt위에 실행하는 jvm과 별도의 jvm에서 실행하게 된다. 이를 하지 않으면 아래와 같은 에러가 발생 한다.  
error message  
{% highlight scala %}
